      ...
      worksheet.columns.forEach((column) => {
        column.width = Math.max(
          ...
          ...sheetData.rows.map((row) => {
            const cellValue = row[worksheet.columns.indexOf(column)];
...
export async function generateMeetingRecap(data: {
  meeting: string;
  date: string;
  attendees: string[];
  summary: string;
...
export async function generateDailyBriefDocument(data: {
  date: string;
  ...
  const sections: WordDocumentSection[] = [
    ...
    },
    {
      type: "list",
      content: "",
      items: data.events.map((event) => `${event.time} - ${event.title}`),
    },
    {
      type: "heading",
    ...
    },
    {
      type: "list",
      content: "",
      items: data.emails.map((email) => `"${email.subject}" from ${email.from}`),
    },
  ];
...
export async function generateStatusDeck(data: {
  title: string;
  wins: string[];
  challenges: string[];
